...
> NOTE: do NOT inline the migration via vite `define`. vpw serializes the worker/test bundle into an HTTP header (`MF-Vitest-Worker-Data`, a ByteString), and non-Latin1 bytes (the migration's Chinese) break it. Integration tests import the schema string from `test/schema.ts` instead (see Task 5).
...
Also create an empty `public/.gitkeep` so the `[assets] directory = "./public"` binding resolves at pool load (the real files arrive in Task 11).

- [ ] **Step 5: Install dependencies**

Run: `npm install`
Expected: completes, `node_modules/` created. (A repo-root `.npmrc` with `legacy-peer-deps=true` keeps the install clean across wrangler/workers-types peer versions.)
...
- [ ] **Step 2a: Create `test/schema.ts` — schema string for tests**

Tests run inside workerd (no fs) and the migration can't be vite-`define`d (its non-Latin1 chars break the workers-pool ByteString header). So expose the SQL as a template literal here, copied verbatim from `migrations/0001_init.sql`. Keep the two in sync.

```ts
export const SCHEMA_SQL = `
(paste the exact, verbatim contents of migrations/0001_init.sql here)
`;
```

- [ ] **Step 2b: Create integration test harness `test/api.test.ts`**
